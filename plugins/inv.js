...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    let ratu medusa = global.db.data.users[m.sender].ratumedusa
    let _ratu medusa = global.db.data.users[m.sender].anakular
    ...
    let anvered = global.db.data.users[m.sender].anvered
    ...
    let sortedanvered = Object.entries(global.db.data.users).sort(a, b) => b[1].anvered - a[1].anvered) 
    ...
    let usersanvered = sortedanvered.map(v => v[0])
    let str = `
...
🐍 Ratu Medusa: *${Ratu Medusa == 0 ? 'Miskin G Ad': '' || ratu medusa == 1 ? 'Ada' : ''}*\n\n
